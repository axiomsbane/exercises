...
lastDigit n
    | n >= 0 && n<= 9 = n
    | otherwise = lastDigit (mod (abs n) 10)
...
lowerAndGreater n list = show n ++ " is greater than " ++ gt ++ " elements and lower than " ++ lt ++ " elements"
    where lt = show $ length $ filter (> n) list
          gt = show $ length $ filter (< n) list 
